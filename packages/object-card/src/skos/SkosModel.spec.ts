...
describe(getLanguageTag.name, () => {
  it('returns only usable language tags for semantic markup', () => {
    expect(getLanguageTag(' nl ')).toBe('nl');
    expect(getLanguageTag('')).toBeUndefined();
    expect(getLanguageTag('?')).toBeUndefined();
  });
});
